...
REQUIRED_UNREAL_REPORT_KEYS = {
    ...
    "height_audit_setup",
    "height_audit_label_category",
    "height_review_target_label_category",
    "height_review_target_count",
    "source_backed_buildings",
    "heuristic_estimated_buildings",
...
REQUIRED_UNREAL_FIRST_PERSON_MARKERS = {
    ...
    "CapitolMap_PlayerStart_SurroundingStreetLevel",
    "CapitolMap_PlayerStart_PublicInteriorWalk",
    "CapitolMap_PlayerStart_Rotunda",
    "CapitolMap_PlayerStart_HouseChamber",
    "CapitolMap_PlayerStart_SenateChamber",
    "CapitolMap_PlayerStart_HouseGallery",
    "CapitolMap_PlayerStart_SenateGallery",
    "CapitolMap_PlayerStart_HouseOfficeWest",
    "CapitolMap_PlayerStart_SenateOfficeEast",
    "CapitolMap_PlayerStart",
    "public_exterior_street_level",
...
REQUIRED_VIEWER_MARKERS = {
    ...
    'id="presetStreetLevel"',
    'id="presetHeightAudit"',
    'id="presetSurroundingRooftops"',
    ...
    'value="facade_detail"',
    'value="building_detail"',
    'value="building_roof_detail"',
    'value="height_audit"',
    'value="height_review_target"',
    'value="office_detail"',
    ...
    "metadata.exterior?.building_details",
    "metadata.exterior?.height_model?.height_review_targets",
    "metadata.landmark?.facade_details",
    "surroundingRooftopBuildingDetailKinds",
    "building_roof_detail",
    "focusStreetLevelRoute",
    "focusStreetLevel",
    "surrounding-building-details",
    "Focused surrounding-building street-level details",
    "focusHeightAuditRoute",
    "focusHeightAudit",
    "building-heights",
    "Focused surrounding-building height audit",
    "focusSurroundingRooftopsRoute",
    "focusSurroundingRooftops",
    "surrounding-rooftops",
    "rooftop-equipment",
    "Focused surrounding-building rooftop details",
    ...
    "focusWalkMap",
    "focusWalkMapRoute",
    "focusWalkStreetLevel",
    "focusWalkStreetLevelRoute",
    "Walk Street mode active",
    "focusWalkInterior",
...
REQUIRED_OFFICE_DETAIL_KINDS = {
    ...
    "generic_office_inbox_tray",
    "generic_office_loose_cable_loop",
    "generic_office_task_lamp",
    "generic_office_partition_panel",
    "generic_office_partition_top_cap",
    "generic_office_monitor_stand",
    "generic_office_desk_cable_grommet",
    "generic_office_cable_tray",
    "generic_office_wall_outlet_plate",
    "generic_office_data_plate",
    "generic_office_chair_back",
    "generic_office_chair_arm_pair",
    "generic_office_chair_swivel_base",
    "generic_office_bookcase",
    "generic_office_book_spine_row",
    "generic_office_storage_cabinet",
    "generic_office_cabinet_label_plate",
    "generic_office_pinboard_panel",
    "shared_support_document_tray",
    "shared_support_supply_bin",
}
...
REQUIRED_UNREAL_INSPECTION_MARKERS = {
    ...
    "generic_non_operational_rooftop_visual",
    "approximate_public_surrounding_building_visual",
    "approximate_public_capitol_visual",
...
def validate_metadata(metadata: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    ...
    if len([prop for prop in streetscape_props if prop.get("kind") == "traffic_signal_pushbutton_plate"]) < 135:
        error(errors, "expected at least 135 historical-source public traffic-signal pushbutton plate props")
    ...
    if len(facade_details) < 7550:
        error(errors, f"expected at least 7550 public facade/furniture visual details, got {len(facade_details)}")
    ...
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_glass_drum"]) < 12:
        error(errors, "expected at least 12 public revolving-door glass drum records")
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_track_ring"]) < 24:
        error(errors, "expected at least 24 public revolving-door track ring records")
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_center_post"]) < 12:
        error(errors, "expected at least 12 public revolving-door center post records")
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_wing_panel"]) < 48:
        error(errors, "expected at least 48 public revolving-door wing panel records")
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_perimeter_mullion"]) < 48:
        error(errors, "expected at least 48 public revolving-door perimeter mullion records")
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_threshold_plate"]) < 12:
        error(errors, "expected at least 12 public revolving-door threshold plate records")
    if len([detail for detail in facade_details if detail.get("kind") == "public_revolving_door_dark_recess"]) < 12:
    ...
    if len([detail for detail in facade_details if detail.get("kind") == "roof_dormer_side_cheek"]) < 32:
        error(errors, "expected at least 32 public roof dormer side-cheek records")
    if len([detail for detail in facade_details if detail.get("kind") == "roof_dormer_glass_highlight"]) < 32:
    ...
    if len(office_details) < 1640:
        error(errors, f"expected at least 1640 public office detail records, got {len(office_details)}")
    ...
    if len([detail for detail in office_details if detail.get("kind") == "shared_support_document_tray"]) < 8:
        error(errors, "expected at least 8 shared support document-tray records")
    if len([detail for detail in office_details if detail.get("kind") == "shared_support_supply_bin"]) < 8:
        error(errors, "expected at least 8 shared support supply-bin records")
    ...
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_inbox_tray"]) < 60:
        error(errors, "expected at least 60 generic office inbox-tray records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_loose_cable_loop"]) < 60:
        error(errors, "expected at least 60 generic office loose-cable loop records")
    ...
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_partition_panel"]) < 180:
        error(errors, "expected at least 180 generic office partition-panel records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_partition_top_cap"]) < 60:
        error(errors, "expected at least 60 generic office partition top-cap records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_monitor_stand"]) < 60:
        error(errors, "expected at least 60 generic office monitor-stand records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_desk_cable_grommet"]) < 60:
        error(errors, "expected at least 60 generic office desk cable-grommet records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_cable_tray"]) < 60:
        error(errors, "expected at least 60 generic office cable-tray records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_wall_outlet_plate"]) < 60:
        error(errors, "expected at least 60 generic office wall outlet-plate records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_data_plate"]) < 60:
        error(errors, "expected at least 60 generic office data-plate records")
    ...
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_book_spine_row"]) < 60:
        error(errors, "expected at least 60 generic office book-spine row records")
    ...
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_cabinet_label_plate"]) < 60:
        error(errors, "expected at least 60 generic office cabinet label-plate records")
    if len([detail for detail in office_details if detail.get("kind") == "generic_office_pinboard_panel"]) < 60:
        error(errors, "expected at least 60 generic office pinboard-panel records")
