...
The live Unreal scenery also includes six generic galvanized bicycle-style temporary crowd-control barricade modules at a public west-approach inspection area. A close-range hardware pass adds rounded caps to both rails, small lower-rail weld collars at the outer uprights, and flat ground-contact plates beneath the two existing tubular feet on every module. Their dimensions and construction remain representative visual scenery, while their placement is explicitly tagged `Public_Schematic_Placement`; it is not presented as a source-verified operational perimeter or security layout.
