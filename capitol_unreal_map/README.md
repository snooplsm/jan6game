...
- surrounding-building height provenance is tracked in metadata: 27 buildings use explicit OSM/DCGIS height tags, 76 use OSM `building:levels * 3.4m` estimates, 4 use authoritative public story-count estimates, 1 uses source-published structural floor dimensions, 1,327 missing-height footprints use conservative DCGIS Planimetrics 1999 rooftop-to-ground elevation deltas, and 2,204 still use deterministic footprint/type/area estimates because they lack clean matched height data; 1,355 surrounding-building heights are now counted as source-backed, every surrounding building records `height_accuracy_tier`, `height_confidence`, and `height_review_priority`, and `height_model.height_review_targets` lists the 40 largest/highest-priority estimated buildings for the next source-matching pass; the checked-in DCGIS source contains 2,360 rooftop elevation points and 2,307 ground elevation points for the current map bounds; each surrounding building now records footprint area and footprint span for height auditing; One Independence Square at 250 E Street SW is now named from public property/GSA records and uses its published nine-floor 27.38m structural stack instead of a 32.83m footprint heuristic; the Hart Senate Office Building now uses the U.S. Senate's nine-story description for a clearly labeled 30.6m level-count estimate instead of its former 13.6m footprint heuristic; the O'Neill House Office Building's unchanged seven-level estimate is now corroborated by the Architect of the Capitol; recovered Longworth and Dirksen footprints use AOC-supported seven-level estimates; the replaced OSM Capitol footprint carries an explicit 87.6m source height, and the authored Capitol visual mesh validates against an 87.48m public-height target
