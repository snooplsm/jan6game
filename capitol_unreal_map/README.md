...
- 3,643 surrounding building footprints from the Jan 6-era OSM extract, including 12 previously omitted single-outer-ring building multipolygons; the recovered civic footprints include the Supreme Court, Cannon, Longworth, Russell, Rayburn, Dirksen, the U.S. Botanic Garden, the Library of Congress Thomas Jefferson Building, the E. Barrett Prettyman Federal Courthouse, and the Henry J. Daly Building; relation outer/inner way IDs are retained in metadata for later courtyard-aware modular replacements; the OSM `United States Capitol` footprint is intentionally skipped and replaced by the authored Capitol landmark mesh to avoid concave-roof triangulation artifacts
...
A project-ready modular lamp candidate is staged at `/Game/HistoricalOSM/Props/LampPostLight/SM_LampPostLight`, with its converted OBJ source, original PBR source maps, and attribution under `SourceAssets/LampPostLight`. It is derived from [Sousinho's "Lamp post light"](https://sketchfab.com/3d-models/lamp-post-light-ff5e3bc99dd146d986cb662813ec988e), licensed CC BY 4.0. The mesh is correctly converted from glTF Y-up to Unreal Z-up in centimeters and uses a conventional four-level LOD chain of 3,808, 1,904, 761, and 305 triangles at screen-size thresholds 1.0, 0.45, 0.18, and 0.06. Two low-cost convex hulls provide first-person collision. Project-owned body and globe materials use the authored base-color and tangent-space normal maps plus glTF-standard packed roughness from the green channel and metallic from the blue channel; the globe remains non-emissive for the fixed daylight scene. An Unreal asset render confirms the upright orientation and intact body/globe material separation. It is intentionally not placed in the live map yet: reversible tests at 4.65m and 2.98m heights showed the current baked rectangular globe protruding around it, while hiding the shared baked lamp slots would also remove hundreds of mapped context streetlights. It should replace the grounds fixtures only after those placeholder faces can be removed cleanly from the combined streetscape mesh.
