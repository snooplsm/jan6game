...
- surrounding-building height provenance is tracked in metadata: 27 buildings use explicit OSM/DCGIS height tags, 76 use OSM `building:levels * 3.4m` estimates, 5 use authoritative public story-count estimates, 1 uses source-published structural floor dimensions, 1,327 missing-height footprints use conservative DCGIS Planimetrics 1999 rooftop-to-ground elevation deltas, and 2,203 still use deterministic footprint/type/area estimates because they lack clean matched height data; 1,355 surrounding-building heights are now counted as source-backed, every surrounding building records `height_accuracy_tier`, `height_confidence`, and `height_review_priority`, and `height_model.height_review_targets` lists the 40 largest/highest-priority estimated buildings for the next source-matching pass; the checked-in DCGIS source contains 2,360 rooftop elevation points and 2,307 ground elevation points for the current map bounds; each surrounding building now records footprint area and footprint span for height auditing; One Independence Square at 250 E Street SW is now named from public property/GSA records and uses its published nine-floor 27.38m structural stack instead of a 32.83m footprint heuristic; the Eric E. Hotung International Law Building now uses Georgetown Law's six-story description for a clearly labeled 20.4m level-count estimate instead of its former 30.24m footprint heuristic; the Hart Senate Office Building now uses the U.S. Senate's nine-story description for a clearly labeled 30.6m level-count estimate instead of its former 13.6m footprint heuristic; the O'Neill House Office Building's unchanged seven-level estimate is now corroborated by the Architect of the Capitol; recovered Longworth and Dirksen footprints use AOC-supported seven-level estimates; the replaced OSM Capitol footprint carries an explicit 87.6m source height, and the authored Capitol visual mesh validates against an 87.48m public-height target
- anonymous way `74035728` at 415 New Jersey Avenue NW is target-date-labeled as the `YOTEL Washington DC transition site`: YOTEL's own timeline says the former Liaison property remained open through a year-long reconstruction and officially debuted on February 1, 2021, placing the January 6 scene 26 days before opening; the later completed rooftop-pool state is not silently backdated, and the existing 32.98m heuristic extrusion remains explicitly unresolved because available `Deck 11` and secondary 13-story descriptions do not establish a reliable total exterior roof height; curated display names are now separated from deterministic source-name height seeds so an evidence-only identity correction cannot accidentally reshape a building
- Capitol Hill Baptist Church now includes a modeled target-era accessibility detail outside the generic facade-distance budget: DC HPRB case `15-502` documents the A Street ramp's 52-foot footprint, six-foot entrance rise, 1:12 slope, matching brick base, and black iron rail, while the church corroborates that the smaller A Street parking lot ramp enters the first floor; the generated assembly has three true sloped deck runs, three landings, six continuous sloped rails, and twelve round posts fitted to the historical southwest footprint edge; because the public staff report does not publish fabrication drawings, the switchback layout is explicitly approximate and the unresolved 24.58m sanctuary/tower heuristic is not promoted
- 12,578 surrounding-building visual detail records, including nearby facade windows, roofline caps, cornice bands, parapet details, entries, entry stoops, ground-floor window panels, blind slats, sill blocks, vents, utility meter banks, service panels, address plaques, downspouts, street-level grime patches, rooftop details, supplemental roof equipment pads, HVAC fan housings, vent clusters, skylight domes, roof drain boxes, roof stain patches, facade pilasters, awnings, wall signs, a source-dimensioned Capitol Hill Baptist accessible-ramp assembly, and public-source height audit labels; recovered civic-building multipolygons are processed before anonymous context footprints so the capped close-range facade and roof detail budget reaches the Supreme Court, congressional office buildings, Library of Congress, Botanic Garden, and federal courthouse massing
- 1,432 roads/paths
...
- 221 authored public grounds detail records, including lawn panels, 10 schematic public lawn slope panels, 28 contour bands, 8 grade-break strips, public walks, reflecting-pool marker, pool coping, formal planting beds, tree allees, 18 Unreal-spawnable public walk lamps, low plaza walls, 12 hedges, 16 path-edge stones, 16 grounds benches, 24 ornamental planting clusters, both historical OSM outer surfaces of the Spirit of Justice Park green roof, and source-aligned House East/West Fountain basin and coping geometry

The two source-aligned House fountains default to a conservative
`winter_inactive` target state: their exact public basin and coping geometry is
retained, but the generated surface uses the stone-basin response and the PCG
Water handoff disables water and active jets. The Architect of the Capitol says
smaller fountains are cleaned to maintain water flows in spring, summer, and
fall, while fall maintenance prepares campus fountains for the colder season;
that supports an inactive January default but does not prove whether either
specific basin was fully dry at 11:50 a.m. on January 6, 2021. A future dated
photograph can override this conservative state without changing geometry.
...
The live dome silhouette now uses a reversible outer-shell correction over the preserved combined mesh. The overlay replaces the visible stepped-cone reading with a broad cylindrical drum, flattened upper hemisphere, transition rings, 36-column/three-tier window articulation, a 12-column/12-window glazed Tholos, cap, pedestal, and simplified Statue of Freedom silhouette. Its exterior shell rhythm now follows the 36 hollow cupola-roof ribs explicitly noted on Thomas U. Walter's circa-1860 drawing in Library of Congress item `93510472`, replacing the former 24-rib simplification and driving 36 visible curved ribs, 36 rib records, and 144 shell panel frames. Each visible rib now follows 24 overlapping, gradually tapered shell-profile samples rather than ten separated blocks, eliminating the former dashed/stepped rib silhouette; validation requires at least 20 samples per documented rib. The two shell-relief seam bands now also occupy the centers of those 36 structural bays, producing 72 aligned seams instead of retaining the obsolete 24-bay pattern. The AOC-documented 36 peristyle columns are now distinct base/shaft/capital assemblies spaced ten degrees apart and positioned wholly outside the 15.4m drum backdrop; this corrects the earlier modeling-order defect that buried their shafts inside the cylinder and made the colonnade read as embossed seams. The exterior window inventory now contains the documented 108 openings as 36 peristyle, 36 second-story, and 36 cupola windows; the cupola tier occupies the shell spring between ribs instead of creating an unsupported additional drum floor. The Tholos now uses the AOC-documented 12 columns and 12 windows at 30-degree spacing, replacing the unsupported 16-column/eight-window lantern rhythm. The hero outer shell is tessellated at 144 radial segments by 32 vertical rings, doubled from 96 by 16, while the lantern cap is 96 by 16 instead of 48 by 8; validation prevents either close-view silhouette from returning to the coarse settings. It is an improved public architectural approximation, not a substitute for a future measured HABS-derived dome model.
...
The former cone-only Statue of Freedom placeholder is preserved but hidden beneath an eight-component bronze public proxy: 32-segment base and lower plinth, a tapered 32-segment draped body, 24-segment head, shaped sword arm, curved 32-segment shield, slope-following sword blade, and tapered helmet crest. Unlike the previous box-and-low-sided-cylinder proxy, the generated geometry now actually contains the shield and sword claimed by its metadata, and its crest reaches the validated 87.48m overall public building-height target. The separate pedestal is no longer collapsed into a thin generic finial: it now spans the AOC-documented 18.5 feet beneath the separately validated 19.5-foot statue, uses four 48-segment base/shaft/taper stages, and carries simplified count-aligned proxies for the documented 12 wreaths and 12 fasces. It improves landmark readability from public ground views but remains explicitly schematic rather than a sculptural replica.
...
The 16 established `Jan2021_LeaflessTree` placements use a curated Quixel Megascans European Hornbeam field-tree derivative instead of the earlier procedural placeholder. Their authored locations, rotations, scale variation, labels, and count are preserved. The same accepted winter derivative now replaces the 44 cylinder-trunk/stacked-canopy blockouts at the generated public grounds allee positions, with deterministic yaw and 12.4-14.8m height variation across the existing 20m spacing. The focused live tree pass verifies the installed dependency before removing an earlier pass and rebuilds those 44 actors idempotently without touching the established trees or species-unknown DCGIS context layer. Project-owned winter material instances enable the pack's seasonal branch/retained-leaf textures, while a project-owned mesh derivative replaces the seven-level imported chain and large impostor atlases with four conventional mesh LODs at 100, 50, 25, and 10 percent triangle density. Only the dependency closure required by this winter `Field_01` derivative is versioned; the unused remainder of the 10.4 GiB source pack is intentionally excluded from Git. The allee use is a high-quality seasonal proxy and does not assert that European Hornbeam was the exact species at every January 2021 position; source-mapped context trees remain species-neutral and are not replaced wholesale.

The 12 long public hedge boxes and 72 ten-sided ornamental shrub cylinders are no longer baked into the combined roads/grounds mesh. Their source-aligned records now expand during Unreal import into 572 modular instances drawn deterministically from all six installed Maxtree Abelia grandiflora variants: 500 hedge-line shrubs at approximately 1.45m spacing plus three varied shrubs in each of the 24 retained stone planting rings. Each variant is scaled from its own mesh bounds to target roughly 0.72m maintained height, while deterministic mesh, yaw, and height variation suppress obvious repetition. The connected [first-person viewport capture](../Docs/validation/shrubs_maxtree_abelia_variants_2026-07-13.png) verifies all six variants loaded, grounded pivots, plausible maintained height, and a balanced 96/96/95/95/95/95 distribution. These remain high-quality semi-evergreen visual proxies, not a claim that Abelia was the exact January 2021 species at every mapped hedge. The importer verifies at least one Fab asset before removing an existing shrub pass, omits collision/navigation for the replacements, and removes/rebuilds them idempotently with the other `CapitolMap` actors.

The Capitol west-front reconstruction has begun at the silhouette level rather than adding more surface detail to the former blockout. A corrected modeling-order pass now actually exposes the front colonnade: because each solid principal portico block is 17m deep, its outer wall lies 8.5m from center, and the prior 2.2m column offset still buried every shaft roughly 6.3m inside it. Both principal porticos now place their ten 48-sided Corinthian-column assemblies at 9.35m center projection, with each 0.72m shaft clearing the wall by 0.13m; the shadow arcade, three revolving-door assemblies, lamps, and sconces share the corrected outer facade plane behind them. The previously blank west back wall now follows the HABS central composition with nine tall recessed glass bays between the columns, two vertical and two horizontal mullions per bay, full jamb/sill/lintel frames, and a projecting 18.6m central stone balcony with 13 turned balusters; the bay rhythm is source-aligned while unsurveyed fabrication dimensions remain approximate. The west approach has three explicit terrace levels and a twelve-tread broad lower stair derived from the stable modern public composition. The dome drum carries a legible 36-bay, three-tier 108-window rhythm and full-height peristyle order, while retaining the official 135 ft exterior diameter, 199 ft skirt-to-Statue-base stack, 288 ft total public height, and documented 36-rib shell rhythm. A second pass adds paired fourteen-tread lateral terrace flights with stone balustrade posts, a thirteen-bay lower terrace arcade, and recessed architectural bay depth across the west-facing House and Senate wing returns. The third pass replaces every rectangular arcade stand-in with a continuous 32-segment extruded arch and tapered keystone, and gives both congressional wing returns distinct projecting three-storey pavilions with five arched-window bays per level, engaged pilasters, and projecting cornices. The fourth pass replaces cylindrical roofline pegs with five-part turned stone balusters, adds slope-following upper and lower rails to the lateral stairs, expands each hero Corinthian capital to twenty acanthus-leaf proxies and four cauliculus stems, and gives the west retaining wall a fifteen-pilaster/fourteen-panel ornamental rhythm. The west silhouette correction removes the invented mirrored East Front triangular pediment and generic sculptural blocks from the central West Front, replacing them with a three-band flat portico crown aligned to the documented west elevation; the AOC explicitly locates the *Genius of America* sculptural pediment at the East Central Entrance. The target-date pass adds the approximately 10,000 sq ft January 6 inaugural platform, its scaffold support grid, the narrowed Lower West Terrace Tunnel, paired white-sheeted stair scaffolds, and temporary seating tiers as an explicitly named modular overlay based on AOC imagery and federal January 6 records. The permanent Olmsted pass adds the documented fourteen West Front lanterns across four style families, preserves the January 2021 four-restored/ten-awaiting-restoration state, models twelve flame-style bulbs per lantern, and adds the hidden west-boundary drinking fountain with its blue-heron focal carving; unsurveyed First Street placements are explicitly marked approximate. This is still a modular reconstruction in progress—accurate carved leaf meshes, surveyed gate and lamp coordinates, east-front sculptural groups, and final Fab stone materials remain below the acceptance gate—but it corrects both the dominant flat-office-facade silhouette and the previously anachronistic open West Front.

The Peace and Garfield Circle lantern pairs are aligned to the actual diagonal-walk connectors in the January 2021 OSM snapshot rather than merely sharing the circles' latitude. Each entrance remains open between the two sandstone lantern piers and now has two flanking low stone walls with marble coping, eighteen bronze rail posts, and continuous top rails. The underlying OSM footways are retained unchanged, avoiding duplicate path surfaces.

The January 6 security overlay is keyed to the requested 11:50 a.m. scene time, before the first documented Peace Circle bypass at about 12:50 p.m. It therefore contains an intact first outer metal-barricade line, an intact second linked bicycle-rack line with dark mesh backing, and an intact north-to-south West Front police line. Each rack module has thirteen uprights, upper/lower rails, two ground feet, weld collars, an unbranded identification plate, and visible plate fasteners. The second Peace Circle line also carries two large white AREA CLOSED panels with red borders, geometry-built bold red lettering, and four rear attachment straps each, matching the wording and broad color treatment documented in federal imagery without generating a sign texture. The target-date inventory is separate from the six older `Public_Schematic_Placement` inspection props, which remain untouched in the live level until a safe editor-side cleanup is performed.
...
The East Central Entrance no longer uses the former nine anonymous relief blocks across an oversized, wall-buried generic pediment. Its `Genius of America` assembly is projected beyond the solid portico wall and now uses the Architect of the Capitol's documented 81 ft 6 in pediment length and 9 ft figure scale, with articulated vertical relief profiles for America, Justice, and Hope instead of the earlier cone/frustum bodies. The USA shield, July 4 1776 altar, Constitution scroll, and spread-wing eagle also use authored facade-plane silhouettes, while the scales and anchor retain thin relief members. This remains an iconography-aligned modular silhouette rather than a claimed scan of the Persico/Mankowski marble carving; a licensed sculpted replacement is still required for first-person acceptance.

The same central group now carries raised geometric marble lettering for the AOC-documented inscriptions: `USA` on America's shield, `JULY 4 / 1776` on its supporting altar, and `CONSTITUTION / 17 SEP 1787` on Justice's scroll. These are deliberately shallow modeled strokes rather than generated text textures, and validation preserves both the wording and substantive stroke counts.

The north and south East Front wing pediments received the same envelope correction: their former 44m-wide, wall-buried generic triangles and fourteen anonymous relief groups are gone. Both now use the AOC's documented 80 ft length, approximate 12 ft center height, and 60 ft sculpture span, projected beyond their solid portico walls. The Senate `Progress of Civilization` record distinguishes Crawford's America, eagle/sun, human-endeavor and early-America groups, wheat, anchor, and grave; the House `Apotheosis of Democracy` record distinguishes Bartlett's Peace and winged Genius, Industry and Agriculture groups, olive tree, torch, livestock, and Atlantic/Pacific waves. These are named distant silhouettes, not substitutes for eventual licensed sculpted hero meshes.

An orientation audit corrected those congressional-wing hero assemblies from the building's north/south side approaches to their documented East Front locations. The generated landmark now has distinct East Senate and East House portico masses at the wing centers, each with six 48-sided, 24-flute Corinthian-column assemblies and an east-facing named pediment. Both porticos carry paired Crawford bronze-door proxies with three narrative panels and one medallion per valve. The Senate set uses AOC's exact 14 ft 5.5 in by 7 ft 4 in dimensions; the House set is explicitly marked as a Senate-comparable visual proxy because the reviewed House source establishes its matching valve/panel composition but not fabrication dimensions. Existing north/south approaches remain separate and no longer carry the East Front pediment art.
